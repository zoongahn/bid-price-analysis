...
5. update_winner_bid_rate - notice 테이블 (winner_bid_rate)
   ※ bid.bid_rate를 참조하므로 update_bid_rates 완료 후 실행
...
def run_all_postprocess(schema: str = None):
    ...
    # 1. notice 테이블 후처리
    try:
        loggers["application"].info("[1/6] notice 테이블 후처리 (answer_rate, min_winning_price)")
        print("\n[1/6] notice 테이블 후처리 (answer_rate, min_winning_price)")
        results["notice"] = update_notice_stats(schema)
        loggers["application"].info("[1/6] notice 테이블 후처리 완료")
    except Exception as e:
        loggers["error"].error(f"[1/6] notice 테이블 후처리 실패: {e}", exc_info=True)
        print(f"[1/6] notice 테이블 후처리 실패: {e}")
        errors.append(("notice", e))
        error_count += 1

    # 2. company 테이블 후처리
    try:
        loggers["application"].info("[2/6] company 테이블 후처리 (has_bid, bid_count)")
        print("\n[2/6] company 테이블 후처리 (has_bid, bid_count)")
        results["company"] = update_company_stats(schema)
        loggers["application"].info("[2/6] company 테이블 후처리 완료")
    except Exception as e:
        loggers["error"].error(f"[2/6] company 테이블 후처리 실패: {e}", exc_info=True)
        print(f"[2/6] company 테이블 후처리 실패: {e}")
        errors.append(("company", e))
        error_count += 1

    # 3. bid 테이블 sucsflwstlmtrt 후처리 (notice.sucsfbidlwltrate로 채움)
    try:
        loggers["application"].info("[3/6] bid 테이블 후처리 (sucsflwstlmtrt)")
        print("\n[3/6] bid 테이블 후처리 (sucsflwstlmtrt)")
        results["bid_sucsflwstlmtrt"] = update_bid_sucsflwstlmtrt(schema)
        loggers["application"].info("[3/6] bid 테이블 sucsflwstlmtrt 후처리 완료")
    except Exception as e:
        loggers["error"].error(f"[3/6] bid 테이블 sucsflwstlmtrt 후처리 실패: {e}", exc_info=True)
        print(f"[3/6] bid 테이블 sucsflwstlmtrt 후처리 실패: {e}")
        errors.append(("bid_sucsflwstlmtrt", e))
        error_count += 1

    # 4. bid 테이블 후처리 (notice.a_value, bid.sucsflwstlmtrt 참조)
    try:
        loggers["application"].info("[4/6] bid 테이블 후처리 (bid_rate, bid_rate_diff)")
        print("\n[4/6] bid 테이블 후처리 (bid_rate, bid_rate_diff)")
        results["bid"] = update_bid_rates(schema)
        loggers["application"].info("[4/6] bid 테이블 후처리 완료")
    except Exception as e:
        loggers["error"].error(f"[4/6] bid 테이블 후처리 실패: {e}", exc_info=True)
        print(f"[4/6] bid 테이블 후처리 실패: {e}")
        errors.append(("bid", e))
        error_count += 1

    # 5. notice 테이블 winner_bid_rate 후처리 (bid.bid_rate 참조)
    try:
        loggers["application"].info("[5/6] notice 테이블 후처리 (winner_bid_rate)")
        print("\n[5/6] notice 테이블 후처리 (winner_bid_rate)")
        results["winner_bid_rate"] = update_winner_bid_rate(schema)
        loggers["application"].info("[5/6] notice 테이블 winner_bid_rate 후처리 완료")
    except Exception as e:
        loggers["error"].error(f"[5/6] notice 테이블 winner_bid_rate 후처리 실패: {e}", exc_info=True)
        print(f"[5/6] notice 테이블 winner_bid_rate 후처리 실패: {e}")
        errors.append(("winner_bid_rate", e))
        error_count += 1

    # 6. notice_industry_type 테이블 후처리
    try:
        loggers["application"].info("[6/6] notice_industry_type 테이블 후처리 (classification_code/name)")
        print("\n[6/6] notice_industry_type 테이블 후처리 (classification_code/name)")
        update_classification_info()  # 이 함수는 schema 파라미터 없음 (내부에서 처리)
        results["notice_industry_type"] = "success"
        loggers["application"].info("[6/6] notice_industry_type 테이블 후처리 완료")
    except Exception as e:
        loggers["error"].error(f"[6/6] notice_industry_type 테이블 후처리 실패: {e}", exc_info=True)
        print(f"[6/6] notice_industry_type 테이블 후처리 실패: {e}")
    ...
    # 성공/실패 요약
    success_count = 6 - error_count
    loggers["application"].info(f"성공: {success_count}/6, 실패: {error_count}/6")
    print(f"성공: {success_count}/6, 실패: {error_count}/6")
