...
COMMENT ON COLUMN reserve_price_range.bssamtBssUpNum IS '기초금액 기준 상향 번호';
COMMENT ON COLUMN reserve_price_range.PrearngPrcePurcnstcst IS '예정가격 순공사비';
COMMENT ON COLUMN reserve_price_range.bidClsfcNo IS '입찰분류번호';
COMMENT ON COLUMN reserve_price_range.rbidNo IS '재입찰번호';
...
COMMENT ON COLUMN reserve_price_range.synced_at IS '마지막 동기화 시점';


-- 복수예가 -> 해당 구간 사정률(총 15개)
-- floor_5dp(user-defined-func.sql)이 UDP로써 미리 정의되어있어야함.
/* GENERATED 컬럼 - floor_5dp 함수가 먼저 정의되어 있어야 함 (user-defined-func.sql) */

ALTER TABLE reserve_price_range
    ADD COLUMN IF NOT EXISTS bssamt_to_bsisplnprc_ratio NUMERIC GENERATED ALWAYS AS (
        CASE
...
ALTER TABLE reserve_price_range
    ADD COLUMN IF NOT EXISTS bssamt_to_bsisplnprc_ratio_floor NUMERIC GENERATED ALWAYS AS (
        CASE
...
COMMENT ON COLUMN reserve_price_range.bssamt_to_bsisplnprc_ratio IS '기초금액 대비 기초예정가격 비율 (%)';
COMMENT ON COLUMN reserve_price_range.bssamt_to_bsisplnprc_ratio_floor IS '기초금액 대비 기초예정가격 비율 (소수점 5자리 내림)';