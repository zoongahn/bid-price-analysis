...
| `reserve_price_range` | `낙찰정보서비스.개찰결과공사예비가격상세목록조회` | `is_synced` |
...
| `ReservePriceRangeSyncer` | reserve_price_range | 단일 프로세스 |
...
> DAG (`scheduler/dags/sync_data_dag.py`)에서도 동일한 순서로 실행됩니다.
...
3. **bizrno 기본값**: 없으면 `__DEFAULT__`로 대체
4. **재실행 안전**: `is_synced=True` 문서는 자동 skip
5. **테스트 시**: `--schema tmp` 사용 권장
